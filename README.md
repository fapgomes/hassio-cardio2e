...
This add-on packages the [cardio2e](https://github.com/fapgomes/cardio2e) bridge. At build time, the release tag pinned in the `Dockerfile` (`ARG CARDIO2E_VERSION`) is cloned from the original repository; every cardio2e release bumps this add-on automatically. At runtime, the add-on reads configuration from the HA UI, generates the config file, and launches the bridge.
